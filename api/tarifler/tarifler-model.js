...
rows.forEach((row) => {
    ...
    {
        adim.icindekiler.push({icindekiler_id:row.malzeme_id, icindekiler_adi: row.malzeme_adi, miktar: row.miktar, birim : row.birim })
        result.adimlar.push(adim)
    }
    
   // eski adım malzeme var
    else { 
        result.adimlar.find(adim => adim.adim_id === row.adim_id)
        .icindekiler
        .push({icindekiler_id:row.malzeme_id, icindekiler_adi: row.malzeme_adi, miktar: row.miktar, birim : row.birim })
    }
});
return result;
}
